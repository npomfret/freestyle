...
const SOURCE_CORPUS_DIR = process.env.SOURCE_CORPUS_DIR
    ? resolve(process.env.SOURCE_CORPUS_DIR)
    : join(ROOT, 'free-stuff');
...
function loadProjects(): Project[] {
    if (!existsSync(SOURCE_CORPUS_DIR)) {
        throw new Error(
            `Legacy source corpus not found at ${SOURCE_CORPUS_DIR}. `
            + 'Normal app usage only needs catalog.json. '
            + 'Set up free-stuff/ or SOURCE_CORPUS_DIR only if you intentionally want to rerun npm run generate.',
        );
    }

    const dirs = readdirSync(SOURCE_CORPUS_DIR)
        .filter((name) => {
            if (name.startsWith('.')) return false;
            return statSync(join(SOURCE_CORPUS_DIR, name)).isDirectory();
        })
        .sort((a, b) => a.toLowerCase().localeCompare(b.toLowerCase()));

    return dirs.map((name) => {
        const dirPath = join(SOURCE_CORPUS_DIR, name);
...
function render(projects: Project[], resources: Resource[]): string {
    ...
    const lines: string[] = [
        ...
        '',
        `This catalog consolidates likely free APIs and public/open datasets from the ${projects.length} source projects in the configured source corpus. It deduplicates resources by URL, keeps direct API/dataset repos, and records which source lists each entry came from.`,
        '',
        'Generated by `src/generate-catalog.ts` from the configured legacy source corpus.',
        ...
        '## Summary',
        '',
        `- Source projects scanned: ${projects.length}`,
...
function main(): void {
    ...
    log.info('catalog generated', {
        ...
        sourceCorpusPath: SOURCE_CORPUS_DIR,
