...
const KNOWN_LIST_PROJECTS = new Set([
  "ai-audio-datasets", "ai-audio-datasets-list", "astro_datasets",
  "climate-change-data", "datasets", "digital-agriculture-datasets",
  "EEG-Datasets", "free-for-dev", "legal-ml-datasets", "open-archaeo",
  "open-access-fMRI-database", "open-computational-neuroscience-resources",
  "open-science-resources", "open-sustainable-technology", "public-api-list",
  "public-api-lists", "public-apis", "public_sport_science_datasets",
  "resources", "security-apis", "The-Databases-for-Drug-Discovery",
  "voice_datasets", "football_analytics",
  "python-resources-for-earth-sciences",
  "papers_for_protein_design_using_DL",
]);

const DIRECT_PROJECTS = new Set([
  "ais-vessel-traffic", "aisstream", "alphafold", "openalex-api-tutorials",
  "opendata", "openelections-core", "openaccess", "POP909-Dataset", "STEAD",
]);

const SKIP_DOMAINS = [
  "img.shields.io", "cdn.jsdelivr.net", "raw.githubusercontent.com",
  "github.com/user-attachments", "visitor-badge.glitch.me",
  "trackgit.com", "zenodo.org/badge", "mybinder.org",
  "colab.research.google.com/assets", "deepnote.com/buttons",
];

const SKIP_URL_PARTS = [
  "/issues", "/pulls", "/actions", "/releases",
  "/blob/main/README", "/blob/master/README",
  "/CONTRIBUTING", "/LICENSE", "/maintainerRole.md", "/free-stuff/",
];

const SKIP_ITEM_LABELS = new Set([
  "link", "github", "website", "paper", "doi", "docs",
  "documentation", "contributing", "contributors",
]);

const GENERIC_LABELS = new Set([
  ...SKIP_ITEM_LABELS, "download", "downloads", "source", "sources",
  "repo", "repository", "home", "homepage", "site",
]);

const NON_RESOURCE_LABEL_PARTS = [
  "about page", "webpage", "documentation", "docs", "tutorial", "course",
  "book", "paper", "survey", "slide", "talk", "video", "blog",
  "newsletter", "community", "conference", "workshop", "forum", "meetup",
  "discord", "slack", "tool", "software", "library", "framework", "sdk",
  "wrapper", "client",
];

const NON_RESOURCE_URL_PARTS = [
  "arxiv.org", "youtube.com", "youtu.be", "medium.com", "substack.com",
  "readthedocs.io", "wikipedia.org", "meetup.com", "discord.gg",
];

const API_HINTS = ["api", "apis", "graphql", "rest", "websocket", "websockets", "openapi"];
const DATASET_HINTS = [
  "dataset", "datasets", "database", "databases", "corpus", "corpora",
  "csv", "json", "parquet", "records", "open-data", "open data",
];

const STRONG_API_SOURCE_PROJECTS = new Set([
  "Awesome_APIs", "aisstream", "free-for-dev", "openalex-api-tutorials",
  "public-api-list", "public-api-lists", "public-apis", "security-apis",
  "awesome-open-source-space-data-apis",
]);

const TYPE_LABELS = new Set([
  "awesome-list", "dataset-list", "api-list", "tooling", "reference",
  "open-data", "codebase", "notebooks", "services", "real-time", "archived",
]);
...
function firstDescription(projectName: string, text: string): string {
  ...
  for (const raw of text.split("\n")) {
    ...
    const badgeWords = [
      "badge", "track awesome list", "visitor", "follow me on x",
...
function labelsFor(projectName: string, description: string): string[] {
  ...
  const labels: string[] = [];
  const add = (l: string) => { if (!labels.includes(l)) labels.push(l); };
...
function isListBased(
  ...
  items: Item[], labels: string[],
): boolean {
  if (DIRECT_PROJECTS.has(projectName)) return false;

  const low = description.toLowerCase();
  const listPhrases = [
    "curated list", "list of", "directory", "collection", "collection of",
    "catalog", "index", "resource list", "dataset list", "resource hub",
  ];
  const datasetPhrases = [
    "list of", "directory", "collection", "catalog", "index",
    "gathers", "compilation",
  ];
...
function shouldSkipUrl(
  projectName: string, repoUrl: string, label: string, url: string,
): boolean {
...
function loadProjects(): Project[] {
  ...
  return dirs.map((name) => {
    ...
    return {
      name, repoUrl, description, labels, listBased,
...
function buildResources(projects: Project[]): Resource[] {
  ...
  function ensure(url: string) {
    if (!catalog.has(url)) {
      catalog.set(url, {
        names: [], kinds: new Set(), topics: new Set(),
        sources: new Set(), descriptions: new Set(),
      });
    }
